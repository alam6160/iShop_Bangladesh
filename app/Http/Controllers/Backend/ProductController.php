...
class ProductController extends Controller
{
    ...
    public function StoreProduct(Request $request)
    {
        ...
        $product_view = auth()->guard('admin')->user()->product_view == 1;
        $product_add = auth()->guard('admin')->user()->product_add == 1;


        if ($product_add == true && $product_view == false) {

            return redirect()->back()->with($notification);
        } elseif ($product_view == true && $product_add == true) {
    ...
    public function MultiImageUpdate(Request $request)
    {
        // Check if multi_img is present in the request and is not null
        if ($request->has('multi_img') && !is_null($request->multi_img)) {
            $imgs = $request->multi_img;
